...
struct Config {
  ...
  size_t get_nvl_buffer_size_hint(size_t hidden_bytes, int num_ranks) const {
    ...
    EP_HOST_ASSERT(num_ranks < num_nvl_peers or num_sms % 2 == 0);
    const auto num_rdma_ranks = std::max(num_ranks / num_nvl_peers, 1);
    const auto num_nvl_ranks = std::min(num_ranks, num_nvl_peers);
  ...
  size_t get_rdma_buffer_size_hint(int64_t hidden_bytes, int num_ranks) const {
    // Legacy mode
    if (num_ranks <= num_nvl_peers) return 0;

    // Below are some assumptions
    // TODO: add assertions
    constexpr int kNumMaxTopK = 128;
    constexpr int kNumMaxScales = 128;
    EP_HOST_ASSERT(num_ranks % num_nvl_peers == 0);
    EP_HOST_ASSERT(num_sms % 2 == 0);
    const int num_rdma_ranks = num_ranks / num_nvl_peers;
    const int num_channels = num_sms / 2;

    size_t num_bytes = 0;
    num_bytes += num_channels * num_rdma_ranks * (num_nvl_peers * 2 + 2) * 2 * sizeof(int);
