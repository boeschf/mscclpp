...
struct Config {
  ...
  size_t get_nvl_buffer_size_hint(size_t hidden_bytes, int num_ranks,
                                  int num_local_ranks = NUM_MAX_NVL_PEERS) const {
    // Below are some assumptions
    // TODO: add assertions
    constexpr int kNumMaxTopK = 128;
    constexpr int kNumMaxScales = 128;
    EP_HOST_ASSERT(num_local_ranks > 0 && num_local_ranks <= NUM_MAX_NVL_PEERS);
  ...
  size_t get_rdma_buffer_size_hint(int64_t hidden_bytes, int num_ranks,
                                   int num_local_ranks = NUM_MAX_NVL_PEERS) const {
    // Legacy mode
    if (num_ranks <= num_local_ranks) return 0;
    ...
    size_t num_bytes = 0;
    num_bytes += num_channels * num_rdma_ranks * (num_local_ranks * 2 + 2) * 2 * sizeof(int);
