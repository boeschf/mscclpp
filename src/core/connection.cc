...
struct OfiConnection::Impl {
  ...
  bool disableWriteDataSignal = false;
...
void OfiConnection::updateAndSync(RegisteredMemory dst, uint64_t dstOffset, uint64_t* src, uint64_t newValue) {
  ...
  *impl_->updateScratch = newValue;
  ...
  if (impl_->useWriteDataSignal && !impl_->disableWriteDataSignal) {
    ...
    for (;;) {
      ...
      if (rc == -FI_EOPNOTSUPP || rc == -FI_ENOSYS || rc == -FI_EINVAL) {
        impl_->disableWriteDataSignal = true;
        WARN(CONN, "fi_writedata unsupported, falling back to CQ-based signal path; rc=", rc);
        break;
      }
    ...
    if (impl_->disableWriteDataSignal) {
      DEBUG(CONN, "OfiConnection updateAndSync: writedata path disabled at runtime, using fallback signaling");
    } else {
